...
} from "./pedagogicalAnalytics.catalog";
...
} from "./pedagogicalAnalytics.interface";
...
} from "./pedagogicalAnalytics.repository";
...
const inferCompetencyIds = (values: string[], explicit: string[] = []) => {
  ...
  /*
   * Fallback: se é claramente um conteúdo de ECG mas nenhuma
   * competência específica foi encontrada, entra em fundamentos.
   */
...
const sourceTypeFromLearningEvent = (
  ...
  switch (event.type) {
    ...
    case "lesson_completed":
      /*
       * Só transforma uma aula em evidência de desempenho
       * se houve avaliação mensurável.
       */
    ...
    /*
     * game_history é a fonte canônica dos jogos para não
...
const weightedMastery = (items: PedagogicalEvidenceItem[], now: Date) => {
  ...
  for (const item of items) {
    ...
    /*
     * Evidências recentes pesam mais, mas uma evidência antiga
     * nunca cai abaixo de 45% do peso de uma recente.
     */
...
const buildContents = (evidence: PedagogicalEvidenceItem[], now: Date) => {
  ...
  for (const item of evidence) {
    /*
     * Se a atividade enviou evidências granulares, elas entram como
     * conteúdos específicos (ex.: "Fibrilação atrial").
     */
    ...
    /*
     * Fallback atual: tags do jogo/atividade. Isso já faz o sistema
     * funcionar com os dados existentes do Rhythmos.
     */
...
const buildImprovement = (
  ...
  if (averageTimeDeltaPercent !== null) {
    /*
     * Menos tempo com precisão/desempenho preservados é um sinal
     * secundário de fluência. Limitamos sua influência.
     */
