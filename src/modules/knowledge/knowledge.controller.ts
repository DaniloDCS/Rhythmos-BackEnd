...
function sanitizeArticleBody(body: Record<string, unknown>) {
  ...
  return {
    ...
    status: ALLOWED_STATUS.has(String(body.status))
      ? String(body.status)
      : "rascunho",
    ...
    ecg:
      body.ecg && typeof body.ecg === "object" ? body.ecg : { enabled: false },
  };
}
...
export const getPublishedKnowledge = async (_req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      error: err instanceof Error ? err.message : String(err),
      message: "Erro ao buscar Atlas ECG.",
    });
  }
};

export const getPublishedKnowledgeBySlug = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const relatedArticles = (
      await Promise.all(
        relatedIds.slice(0, 8).map(async (relatedId) => {
          ...
          if (!related.exists || related.data()?.status !== "publicado")
            return null;
    ...
    return res.status(500).json({
      error: err instanceof Error ? err.message : String(err),
      message: "Erro ao buscar ficha publicada.",
    });
  }
};
