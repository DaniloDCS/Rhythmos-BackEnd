...
import type { IUserProgress } from "../../interfaces/IUserProgress";
import type { ILevel } from "../../models/Levels";
import { calculateUpdatedStreak } from "../../controllers/UserProgress.controller";
...
const normalizeEcg = (value: unknown): IClinicalCaseECG => {
  const ecg = value && typeof value === "object"
  ...
  return {
    ...
    pathological,
    paperSpeed:
      Number.isFinite(paperSpeed) && paperSpeed > 0 ? paperSpeed : 25,
...
const validateSteps = (steps: IClinicalCaseStep[]) => {
  ...
  if (ids.size !== 10 || orders.size !== 10) {
    throw new ClinicalError(400, "Os 10 passos precisam ter IDs e ordens únicos.");
  }
};
...
const validateAnalysis = (item: IClinicalCase, analysis: ClinicalStepAnswers) => {
  const steps = expectedSteps(item);
  if (steps.length !== 10) {
    throw new ClinicalError(500, "O caso não possui 10 passos de análise configurados.");
  }
...
export class ClinicalService {
  ...
  private async ensureSequence(sequence: number, ignoreId?: string) {
    ...
    const duplicate = await clinicalRepository.findBySequence(sequence, ignoreId);
    if (duplicate) {
  ...
  async answer(
    ...
    if (!answer.trim()) throw new ClinicalError(400, "A resposta final é obrigatória.");
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const alreadyCompleted = completionDoc.exists && completionDoc.data()?.completed === true;
      ...
      // Ainda não houve nenhuma escrita nesta transaction, então podemos ler o progresso.
      const progressDoc = await transaction.get(userProgressRef);
      if (!progressDoc.exists) {
        throw new ClinicalError(404, "Progresso global do usuário não encontrado.");
      }
      ...
      const currentLevel =
        [...levels].reverse().find((level) => newXp >= level.xpMin) ?? levels[0];
