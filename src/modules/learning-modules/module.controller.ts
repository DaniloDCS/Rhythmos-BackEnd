...
export const getPublishedModules = async (_: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar módulos publicados",
...
export const getAccessibleModulesByTrail = async (
  ...
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar módulos acessíveis da trilha",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
...
export const getModuleById = async (req: Request, res: Response) => {
  try {
    ...
    if (!isAdminView) {
      ...
      if (userId) {
        ...
        if (!module.canUserAccess(completedModuleIds, allModules)) {
          return res.status(403).json({
            error: "FORBIDDEN",
            message: "Módulo bloqueado por pré-requisitos",
          });
        }
      }
    ...
    return res.status(500).json({
      message: "Erro ao buscar módulo pelo ID",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
