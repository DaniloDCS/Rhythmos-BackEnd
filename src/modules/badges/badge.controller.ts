...
import { syncUserBadges } from "./badge-award.service";
...
export const deleteBadgeAdmin = async (req: Request, res: Response) => {
  try {
    ...
    const rewardSnapshot = await db
      ...
      .get();
    if (rewardSnapshot.docs.some((item) => item.data().type === "badge")) {
...
export const getActiveBadges = async (_: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar insígnias ativas",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
...
export const getMyBadges = async (
  ...
  try {
    ...
    const result = await syncUserBadges(req.user.uid);
    return res.status(200).json({
      badges: result.catalog,
    ...
    return res.status(500).json({
      ...
      message: "Erro ao carregar insígnias.",
    });
  }
};
