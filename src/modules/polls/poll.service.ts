...
import type { IBlock } from "../lessons/block.types";
...
export class PollService {
  ...
  static async vote(input: {
    ...
    const validOptionIds = new Set(
      block.content.options.map((option) => option.id),
    );
    const optionIds = Array.isArray(input.optionIds)
      ? [
          ...new Set(
            input.optionIds.map(String).filter((id) => validOptionIds.has(id)),
          ),
        ]
      : [];

    if (!optionIds.length)
      throw pollError(400, "Selecione pelo menos uma opção.");
