...
const toNormalizedPoints = (
  ...
  const maxAbs =
    Math.max(...signal.map((value) => Math.abs(value)), 1) || 1;
...
const makeSinusSignal = (
  ...
  for (let index = 0; index < samples; index += 1) {
    ...
    if (options?.baselineWave) {
      value +=
        Math.sin(x * Math.PI * 30) * 0.035 +
        Math.sin(x * Math.PI * 47) * 0.018;
    }
...
const makeAtrialFibrillationSignal = (
  samples: number,
) => {
...
const makeVentricularTachycardiaSignal = (
  samples: number,
) => {
  ...
  switch (rhythmId) {
    ...
    case "sinus_tachycardia":
      return toNormalizedPoints(
        makeSinusSignal(
          samples,
          [0.08, 0.245, 0.41, 0.575, 0.74, 0.905],
          {
            pAmplitude: 0.16,
            tAmplitude: 0.23,
          },
    ...
    case "atrial_fibrillation":
      return toNormalizedPoints(
        makeAtrialFibrillationSignal(samples),
      );

    case "ventricular_tachycardia":
      return toNormalizedPoints(
        makeVentricularTachycardiaSignal(samples),
      );
  }
};
