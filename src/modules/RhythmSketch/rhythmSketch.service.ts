...
import {
  RHYTHM_SKETCH_IDS,
  RHYTHM_SKETCH_RULES,
} from "./rhythmSketch.rules";
import { validateRhythmSketch } from "./rhythmSketch.validator";

const CHALLENGES_COLLECTION =
  "rhythm_sketch_challenges";

const ATTEMPTS_COLLECTION =
  "rhythm_sketch_attempts";
...
export class RhythmSketchService {
  async createChallenge(
    userId: string,
  ): Promise<RhythmSketchChallenge> {
    const rhythmId =
      RHYTHM_SKETCH_IDS[
        Math.floor(
          Math.random() * RHYTHM_SKETCH_IDS.length,
        )
      ];
  ...
  async validate(
    ...
    const challenge =
      challengeDoc.data() as RhythmSketchStoredChallenge;
    ...
    const rule =
      RHYTHM_SKETCH_RULES[challenge.rhythmId];
    ...
    const result = validateRhythmSketch(
      points,
      rule,
    );
    ...
    batch.set(attemptRef, {
      ...
      /*
       * Não salvamos todos os pontos brutos por padrão para evitar
       * crescimento desnecessário da coleção. Eles continuam sendo
       * processados durante a validação.
       */
...
export const rhythmSketchService =
  new RhythmSketchService();
