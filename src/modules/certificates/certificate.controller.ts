...
type CertificateSourceData = {
  ...
  academicIndices?: Awaited<ReturnType<typeof AcademicIndicesService.calculate>>;
};
...
const buildCertificateSourceData = async (
  ...
  const [trailDoc, userDoc, programSnapshot, academicIndices] = await Promise.all([
    ...
    AcademicIndicesService.calculate(enrollment),
  ]);
  if (!trailDoc.exists) {
...
const hydrateLegacyCertificate = async (
  ...
  if (hasProgram && hasWorkload && hasCounts && hasDates && certificate.academicIndices) {
    return certificate;
  }
  ...
  if (!certificate.academicIndices && source.academicIndices) patch.academicIndices = source.academicIndices;
    ...
    const result = await db.runTransaction<CertificateTransactionResult>(
      async (tx) => {
        ...
        const certificate: ICertificate = {
          ...
          academicIndices: source.academicIndices,
        };
...
export const validateCertificate = async (
  ...
  try {
    ...
    const publicCertificate = {
      ...
      academicIndices: certificate.academicIndices,
