...
export class PedagogicalAnalyticsRepository {
  async getGameHistory(userId: string): Promise<RawGameHistory[]> {
  ...
  async getLearningEvents(userId: string): Promise<RawLearningEvent[]> {
    const snapshot = await db
  ...
  async getGamesByIds(
    ...
    const refs = ids.map((id) => db.collection("games").doc(id));

    const snapshots = await db.getAll(...refs);

    const map = new Map<string, RawGameDefinition>();
    ...
    for (const snapshot of snapshots) {
      ...
      map.set(snapshot.id, {
        id: snapshot.id,
        ...(snapshot.data() as Omit<RawGameDefinition, "id">),
      });
    }
