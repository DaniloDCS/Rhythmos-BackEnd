...
const uniqueUsers = (items: AnyRecord[]) =>
  new Set(items.map((item) => String(item.userId ?? "").trim()).filter(Boolean))
    .size;
...
const countInvalidUrls = (value: unknown, keyName = ""): number => {
  if (Array.isArray(value)) {
    return value.reduce(
      (total, item) => total + countInvalidUrls(item, keyName),
      0,
    );
  }
...
const resolveAuditActors = async (
  ...
  docs.forEach((doc) => {
    ...
    actorNames.set(
      doc.id,
      String(
        data.name ?? data.displayName ?? data.username ?? data.email ?? doc.id,
      ),
    );
  });
  return actorNames;
...
export class TrailAnalyticsService {
  async getAnalytics(trailId: string): Promise<ITrailAdminAnalytics> {
    ...
    const [
      ...
      db
        .collection("experience_feedback")
        .where("targetId", "==", trailId)
        .get(),
    ...
    const feedback = (feedbackSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    })) as AnyRecord[]).filter((item) => item.targetType === "trail");
    ...
    const academicIndices = await Promise.all(
      enrollments.map((item) =>
        AcademicIndicesService.calculate(
          item as unknown as IEnrollment & { id: string },
        ),
      ),
    );
    const academicValues = (
      selector: (item: (typeof academicIndices)[number]) => number | null,
    ) =>
      academicIndices
        .map(selector)
        .filter(
    ...
    const latestCohort = academicIndices.find((item) => item.cohortSize > 0);
    const academicPerformance = {
      students: academicIndices.length,
      withNumericAverage: academicIndices.filter(
        (item) => item.completionAverage !== null,
      ).length,
      averageCompletionAverage: average(
        academicValues((item) => item.completionAverage),
      ),
      averageNormalizedCompletionAverage: average(
        academicValues((item) => item.normalizedCompletionAverage),
      ),
      averageWorkloadEfficiencyIndex: average(
        academicValues((item) => item.workloadEfficiencyIndex),
      ),
      averagePeriodEfficiencyIndex: average(
        academicValues((item) => item.periodEfficiencyIndex),
      ),
      averageAcademicEfficiencyIndex: average(
        academicValues((item) => item.academicEfficiencyIndex),
      ),
      averageNormalizedAcademicEfficiencyIndex: average(
        academicValues((item) => item.normalizedAcademicEfficiencyIndex),
      ),
      cohortMean: latestCohort?.cohortMean ?? null,
      cohortSampleStandardDeviation:
        latestCohort?.cohortSampleStandardDeviation ?? null,
      cohortSize: latestCohort?.cohortSize ?? 0,
    };
    ...
    const completionTimes = completedEnrollments
      .map((item) => {
        ...
        return started && finished && finished >= started
          ? finished - started
          : null;
      })
    ...
    const mappedAudit: ITrailAuditItem[] = allAudit
      .map((item) => ({
        ...
        entityType: (item.entityType ??
          "trail") as ITrailAuditItem["entityType"],
    ...
    const prerequisiteDocs = await Promise.all(
      prerequisiteIds.map((id: string) =>
        db.collection("trails").doc(id).get(),
      ),
    );
    const invalidPrerequisites = prerequisiteDocs.filter(
      (doc) => !doc.exists,
    ).length;
    const activityReferences = collectActivityReferences(modules);
    const activityDocs = await Promise.all(
      activityReferences.map((ref) =>
        db.collection(ref.collection).doc(ref.id).get(),
      ),
    );
    const nonexistentActivities = activityDocs.filter(
      (doc) => !doc.exists,
    ).length;
    ...
    const averageAttempts = attemptValues.length
      ? average(attemptValues)
      : null;
    ...
    if (nonexistentActivities)
      warnings.push(
        `${nonexistentActivities} atividade(s) referenciada(s) não existe(m).`,
      );
    if (invalidUrls)
      warnings.push(`${invalidUrls} URL(s) armazenada(s) inválida(s).`);
    let baseProblems = 0;
    ...
    const contentUpdatedDates = [
      ...
      ...modules.map((module) =>
        timestampToMillis(module.updatedAt ?? module.createdAt),
      ),
      ...lessons.map((lesson) =>
        timestampToMillis(lesson.updatedAt ?? lesson.createdAt),
      ),
    ].filter((value): value is number => value !== null);
    ...
    return {
      ...
      complaintCount: feedback.filter((item) => item.type === "complaint")
        .length,
      updatedAt: effectiveUpdatedAt,
      createdBy: createdById || null,
      createdByName: createdById
        ? (actorNames.get(createdById) ?? createdById)
        : null,
      updatedBy: updatedById || null,
      updatedByName: updatedById
        ? (actorNames.get(updatedById) ?? updatedById)
        : null,
      publicationHistory,
