...
export const getPublishedTrails = async (_: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar trilhas publicadas",
...
export const getAvailableTrailsForUser = async (
  ...
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar trilhas disponíveis",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
...
export const getUserTrailsWithProgress = async (
  ...
  try {
    ...
    return res.status(500).json({
      message: "Erro ao buscar trilhas do usuário",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
...
export const getTrailWithModulesAndLessons = async (
  ...
  try {
    ...
    return res.status(500).json({
      message: "Erro ao montar trilha",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};

export const getTrailProgrammaticContent = async (
  ...
  try {
    ...
    if (!trailId) {
      return res.status(400).json({
        error: "VALIDATION_ERROR",
        message: "ID da trilha é obrigatório.",
      });
    }
    ...
    if (trailData.status !== "disponivel") {
      const enrollmentSnapshot = req.user
        ? await db.collection("enrollments").where("userId", "==", req.user.uid).where("trailId", "==", trailId).limit(1).get()
        : null;
      if (!enrollmentSnapshot || enrollmentSnapshot.empty) {
        return res.status(403).json({
          error: "FORBIDDEN",
          message: "Esta trilha não está disponível.",
        });
      }
    }
    ...
    const modules = await Promise.all(
      modulesSnapshot.docs.map(async (moduleDoc) => {
        ...
        if (moduleData.status !== "disponivel") {
          return null;
        }
        ...
        const lessons = await Promise.all(
          lessonsSnapshot.docs.map(async (lessonDoc) => {
            ...
            /*
             * Só disponibiliza aulas publicadas/disponíveis.
             */
        ...
        return {
          ...
          workloadHours: moduleData.workloadHours,
    ...
    return res.status(200).json({
      ...
      workloadHours: trailData.workloadHours ?? 0,

      estimatedMinutes: trailData.estimatedMinutes ?? 0,
    ...
    console.error("Erro ao buscar conteúdo programático:", err);

    return res.status(500).json({
      message:
        err instanceof Error
          ? err.message
          : "Erro ao buscar conteúdo programático.",
      error: err instanceof Error ? err.message : String(err),
    });
  }
};
