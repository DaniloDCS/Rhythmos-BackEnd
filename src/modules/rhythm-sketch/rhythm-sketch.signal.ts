import type { RhythmSketchPoint } from "./rhythm-sketch.types";
...
export const drawingCoverage = (points: RhythmSketchPoint[]) => {
...
const collapseByX = (points: RhythmSketchPoint[]) => {
...
export const resampleDrawing = (
  ...
  for (let index = 0; index < samples; index += 1) {
    const target = minX + (index / Math.max(1, samples - 1)) * width;

    while (cursor < collapsed.length - 2 && collapsed[cursor + 1].x < target) {
      cursor += 1;
    }

    const left = collapsed[cursor];
    const right = collapsed[Math.min(cursor + 1, collapsed.length - 1)];

    const interval = Math.max(0.000001, right.x - left.x);

    const ratio = clamp((target - left.x) / interval, 0, 1);
...
export const referenceToSignal = (points: RhythmSketchPoint[]) =>
  normalizeSignal(points.map((point) => 0.5 - point.y));
...
export const normalizeSignal = (values: number[]) => {
  ...
  const centered = values.map((value) => value - median);

  const maxAbs =
    Math.max(...centered.map((value) => Math.abs(value)), 0.0001) || 1;

  return centered.map((value) => value / maxAbs);
};

export const smoothSignal = (values: number[], radius = 2) => {
...
export const detectDominantPeaks = (
  ...
  const minimumDistance = Math.max(
    8,
    Math.floor(signal.length / Math.max(4, expectedMaxCycles * 1.55)),
  );
  ...
  for (let index = 2; index < signal.length - 2; index += 1) {
    ...
    while (right < signal.length - 1 && signal[right] > half) {
      right += 1;
    }
  ...
  for (const candidate of [...candidates].sort((a, b) => b.value - a.value)) {
    const tooClose = selected.some(
      (peak) => Math.abs(peak.index - candidate.index) < minimumDistance,
    );
...
export const regularityCvFromPeaks = (peaks: DetectedPeak[]) => {
  if (peaks.length < 3) return 0;

  const rr: number[] = [];

  for (let index = 1; index < peaks.length; index += 1) {
    rr.push(peaks[index].index - peaks[index - 1].index);
  }
...
export const averagePeakWidth = (
  ...
  return mean(peaks.map((peak) => peak.width)) / sampleCount;
};

export const signalAmplitude = (signal: number[]) => {
  if (!signal.length) return 0;

  return Math.max(...signal) - Math.min(...signal);
};

export const dtwDistance = (a: number[], b: number[]) => {
  if (!a.length || !b.length) return 1;

  const previous = new Array<number>(b.length + 1).fill(
    Number.POSITIVE_INFINITY,
  );

  const current = new Array<number>(b.length + 1).fill(
    Number.POSITIVE_INFINITY,
  );
  ...
  for (let i = 1; i <= a.length; i += 1) {
    ...
    for (let j = 1; j <= b.length; j += 1) {
      ...
      current[j] =
        cost + Math.min(current[j - 1], previous[j], previous[j - 1]);
    }
...
export const correlation = (a: number[], b: number[]) => {
...
const resampleArray = (values: number[], samples: number) => {
  ...
  for (let index = 0; index < samples; index += 1) {
    const position = (index / Math.max(1, samples - 1)) * (values.length - 1);
    ...
    result.push(
      values[leftIndex] + (values[rightIndex] - values[leftIndex]) * ratio,
    );
  }
...
export const beatConsistencyScore = (
  ...
  const rr =
    peaks.length > 1
      ? mean(
          peaks.slice(1).map((peak, index) => peak.index - peaks[index].index),
        )
      : signal.length / 5;
  ...
  for (const peak of peaks) {
    ...
    const end = Math.min(signal.length, peak.index + after);

    if (end - start < 10) continue;

    segments.push(resampleArray(signal.slice(start, end), 64));
  }
  ...
  for (let index = 1; index < segments.length; index += 1) {
    correlations.push(
      Math.max(0, correlation(segments[index - 1], segments[index])),
    );
  }

  return clamp(mean(correlations) * 100, 0, 100);
};

export const clampScore = (value: number) => Math.round(clamp(value, 0, 100));
