...
import { buildRhythmReference } from "./rhythm-sketch.templates";
import {
  averagePeakWidth,
  ...
  signalAmplitude,
} from "./rhythm-sketch.signal";
const SAMPLE_COUNT = 256;
const PASS_SCORE = 70;
const PERFECT_SCORE = 92;
const MIN_COVERAGE = 0.78;
const MIN_POINTS = 70;
const scoreCycleCount = (detected: number, rule: RhythmSketchRule) => {
  if (detected <= 0) return 0;
  if (
    detected >= rule.expectedCycles.min &&
    detected <= rule.expectedCycles.max
  ) {
    const distance = Math.abs(detected - rule.expectedCycles.ideal);
...
const scoreRegularity = (cv: number, rule: RhythmSketchRule) => {
...
const scoreQrsWidth = (userWidth: number, referenceWidth: number) => {
  if (userWidth <= 0 || referenceWidth <= 0) {
    return 45;
  }
  const ratio =
    Math.min(userWidth, referenceWidth) / Math.max(userWidth, referenceWidth);
  return clampScore(ratio * 100);
};
const morphologyScore = (userSignal: number[], referenceSignal: number[]) => {
  const distance = dtwDistance(userSignal, referenceSignal);
  return clampScore(100 * Math.exp(-2.15 * distance));
};
const buildFeedback = (
  ...
  if (rule.pWave === "present") {
    feedback.push({
      type: breakdown.morphology >= 70 ? "info" : "warning",
      label: "Ondas P",
      detail:
    ...
    feedback.push({
      ...
      detail: "Evite ondas P organizadas e repetitivas antes de cada QRS.",
    });
  }
  if (score >= PASS_SCORE) {
    feedback.unshift({
...
export const validateRhythmSketch = (
  ...
  const referencePoints = buildRhythmReference(rule.id, SAMPLE_COUNT);
  const userSignal = resampleDrawing(points, SAMPLE_COUNT);
  const referenceSignal = referenceToSignal(referencePoints);
  const userPeaks = detectDominantPeaks(userSignal, rule.expectedCycles.max);
  const referencePeaks = detectDominantPeaks(
    referenceSignal,
    rule.expectedCycles.max,
  );
  const detectedCycles = userPeaks.length;
  const regularityCv = regularityCvFromPeaks(userPeaks);
  const userPeakWidth = averagePeakWidth(userPeaks, SAMPLE_COUNT);
  const referencePeakWidth = averagePeakWidth(referencePeaks, SAMPLE_COUNT);
  const cycleScore = scoreCycleCount(detectedCycles, rule);
  const widthScore = scoreQrsWidth(userPeakWidth, referencePeakWidth);
  const structure = clampScore(cycleScore * 0.72 + widthScore * 0.28);
  const timing = clampScore(
    scoreRegularity(regularityCv, rule) * 0.72 + cycleScore * 0.28,
  );
  const morphology = morphologyScore(userSignal, referenceSignal);
  const beatConsistency = beatConsistencyScore(userSignal, userPeaks);
  const consistency = clampScore(
    rule.regularity === "regular"
      ? beatConsistency * 0.72 + scoreRegularity(regularityCv, rule) * 0.28
      : beatConsistency * 0.58 + scoreRegularity(regularityCv, rule) * 0.42,
  );
  const breakdown = {
    structure,
    timing,
  ...
  let score = clampScore(
    structure * 0.4 + timing * 0.25 + morphology * 0.25 + consistency * 0.1,
  );
  if (points.length < MIN_POINTS || coverage < MIN_COVERAGE) {
  ...
  const features = {
    ...
    regularityCv: Number(regularityCv.toFixed(3)),
    dominantPeakCount: userPeaks.length,
    amplitude: Number(signalAmplitude(userSignal).toFixed(3)),
  };
  const passed =
    score >= PASS_SCORE &&
  ...
  const perfect = passed && score >= PERFECT_SCORE;
  return {
    score,
    passed,
    perfect,
    breakdown,
    features,
    feedback: buildFeedback(rule, score, breakdown, features),
    referencePoints,
  };
};
