...
export const ProgressGetAll = async (req: Request, res: Response) => {
  try {
...
export const MyProgress = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    /*
     * Compatibilidade/migração automática:
     * ao carregar o próprio progresso, sincronizamos as recompensas
     * vinculadas aos níveis que ele já alcançou.
     *
     * Isso cobre usuários criados antes da introdução do mapa `unlocked`
     * e também recompensas adicionadas retroativamente a níveis existentes.
     */
    const levelsSnapshot = await db
      .collection("levels")
      .where("active", "==", true)
...
export const ProgressAddXp = async (
  ...
  try {
    ...
    if (levelsSnapshot.empty) {
      return res.status(500).json({ message: "Nenhum nível foi cadastrado." });
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      /*
       * Sincroniza todas as recompensas de níveis já alcançados.
       * O RewardService é idempotente, então isso também repara
       * usuários antigos e permite vincular uma recompensa
       * retroativamente a um nível já atingido.
       */
...
export const ProgressRegisterActivity = async (
  ...
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      /*
       * Se já houve atividade hoje,
       * não precisamos atualizar o banco.
       */
