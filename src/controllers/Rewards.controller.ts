...
import { Reward, type RewardType } from "../models/Rewards";
...
export const createReward = async (
  ...
  try {
    ...
    return res.status(500).json({
      error: err instanceof Error ? err.message : "Erro ao criar recompensa",
    });
  }
};
...
/**
 * Opções amigáveis para o formulário do admin. O frontend nunca precisa
 * pedir que o administrador copie IDs manualmente.
 */
export const getRewardTargetsAdmin = async (_: Request, res: Response) => {
  try {
    const [gamesSnapshot, trailsSnapshot, modulesSnapshot] = await Promise.all([
    ...
    return res.status(500).json({
      error: "Erro ao carregar destinos de recompensa",
    });
  }
};
...
export const deleteRewardAdmin = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: "Erro ao excluir recompensa" });
  }
};
...
export const getActiveRewards = async (_: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: "Erro ao buscar recompensas ativas" });
  }
};
