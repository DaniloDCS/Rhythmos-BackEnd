  ...
  return answers
    ...
    .map((item) => ({
      ...
      optionIds: Array.isArray(item.optionIds)
        ? [...new Set(item.optionIds.map(String).map((id) => id.trim()).filter(Boolean))]
        : [],
    }))
...
export class AssessmentService {
  static async getById(id: string): Promise<Assessment> {
    ...
    if (!snapshot.exists) throw assessmentError(404, "Avaliação não encontrada.");
  ...
  static async submit(input: {
    ...
    if (currentStep.enrollment.trailId !== assessment.trailId) {
      throw assessmentError(403, "Esta avaliação não pertence à matrícula informada.");
    }
    ...
    if (assessment.scope === "lesson") {
      if (
        ...
        throw assessmentError(409, "Esta avaliação não corresponde à aula atual.");
      }
    } else if (
    ...
    const userAttempts = attemptsSnapshot.docs.filter((doc) => {
      ...
      return (
        ...
        data.enrollmentId === input.enrollmentId
      );
    });
    ...
    if (
      ...
      throw assessmentError(409, "O limite de tentativas desta avaliação foi atingido.");
    }
    ...
    const pointsAvailable = results.reduce((sum, result) => sum + result.pointsAvailable, 0);
    const pointsEarned = results.reduce((sum, result) => sum + result.pointsEarned, 0);
    const score = pointsAvailable <= 0 ? 0 : Math.round((pointsEarned / pointsAvailable) * 100);
    ...
    let flowResult: Awaited<ReturnType<typeof LearningFlowService.completeModuleAssessment>> | undefined;
  ...
  static async update(id: string, data: Partial<IAssessment>, userId?: string) {
    ...
    await db.collection(ASSESSMENTS).doc(id).set(assessment.toObject(), { merge: false });
    return assessment;
  }
