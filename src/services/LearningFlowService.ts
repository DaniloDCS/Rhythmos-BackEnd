...
import type { ILesson2, ILessonVersion2 } from "../models/Lesson";
import type { IModule } from "../models/Module";
import {
  LearningFlowResult,
  LearningStep,
} from "../interfaces/LearningFlow.interface";
...
export class LearningFlowService {
  ...
  private static async finishLesson(
    ...
    /*
     * A partir daqui o fluxo precisa obrigatoriamente
     * ter uma próxima etapa definida.
     *
     * Mantemos `nextStep` como null durante a decisão
     * para evitar concluir a trilha por engano caso um
     * novo caminho seja adicionado no futuro.
     */
    if (nextStep === null) {
      throw flowError(
        500,
