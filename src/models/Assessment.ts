...
export class Assessment implements IAssessment {
  ...
  validate() {
    ...
    if (this.status === "disponivel" && this.questions.length === 0) {
      throw new Error("Uma avaliação disponível precisa ter pelo menos uma questão.");
    }

    for (const question of this.questions) {
      if (!question.id) throw new Error("Toda questão precisa de id.");
      if (!question.prompt) throw new Error("Toda questão precisa de enunciado.");
      if (question.options.length < 2) {
        throw new Error(`A questão "${question.prompt}" precisa de ao menos duas opções.`);
      }
      if (!question.options.some((option) => option.correct)) {
        throw new Error(`A questão "${question.prompt}" precisa de resposta correta.`);
      }
      if (
        question.type !== "multiple" &&
        question.options.filter((option) => option.correct).length !== 1
      ) {
        throw new Error(`A questão "${question.prompt}" aceita apenas uma resposta correta.`);
      }
    }
  }
