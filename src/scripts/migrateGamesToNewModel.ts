...
export const migrateGamesToNewModel = async () => {
  ...
  for (const doc of snapshot.docs) {
    ...
    /*
     * Antes existiam `active` + `status`.
     * Para não publicar acidentalmente um jogo antigo que estava inativo,
     * convertemos active=false + disponivel para indisponivel.
     */
    ...
    batch.set(
      doc.ref,
      {
        ...
        xpReward: Math.max(
          0,
          Number(data.xpReward ?? data.xpBaseReward ?? 0),
        ),
